...
def extract_attn_probabilities_true_false_tokens(
    heads: dict,
    models_output: any,
    token_number_sentence: int,
    prints_enabled: bool = False,
...
def plot_logit_probs_all(data_with_folders, results_path):
    ...
    output_file = Path(results_path) / "ablation_layers_experiment_5.pdf"
...
def plot_ablation_results(data_with_folders, results_path):
    ...
    # Save the plot in the parent directory
    output_file = Path(results_path) / "ablation_layers_experiment_5_with_all_knockout.pdf"
    plt.savefig(output_file, format="pdf", bbox_inches="tight", dpi=300)
    print(f"Saved plot: {output_file}")
...
def knock_out_all_attention_layers(layers: list, model):
    # note that this function modifies the model in place and does not return anything
    # the changes you make are not recoverable, so you may want to reload the model to undo them
    for layer in layers:
...
def main():
    ...
    try:
        ...
        results_path = f"results-{models[0]}-new-prompt"
        ...
        if knockout:
            for layer in unique_layers:
                # Run the experiment, knocking out layer by layer and save results.
                run_experiment(
                    dataset_csv_file_path=CSV_PATH_DATASET,
                    model_name=models[0],
                    results_path=results_path,
                ...
                # print(f"The top heads for the experiment after knocking out layer {layer} are {knockout_top_heads}.")
                delete_model(model=model, tokenizer=tokenizer)
                model, tokenizer = initialize_model(model_name=models[0])  # reinit the model

            # Experiment with all layers knocked out
            print("Running experiment with all top layers knocked out.")
            knock_out_all_attention_layers(unique_layers, model)
            run_experiment(
                dataset_csv_file_path=CSV_PATH_DATASET,
                model_name=models[0],
                results_path=results_path,
