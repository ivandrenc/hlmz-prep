...
def plot_logit_probs_all(data_with_folders, results_path):
    ...
    output_file = Path(results_path) / "all_subfolders_logit_probs.png"
...
def run_experiment(dataset_csv_file_path: str, model_name: str, results_path: str, knockout_heads: bool = False, layer: int = None, head: int = None):
    print(
    ...
    # print(f"The top heads with the avg induction score for the second variant are: {top_heads}\n")

    if not top_heads_switched.keys() == top_heads.keys():
        top_heads.update(top_heads_switched)
...
def main():
    ...
    results_path = "results_meta_layers_knockout"
    ...
    for l_h in top_heads.keys():
        ...
        knockout_top_heads = run_experiment(dataset_csv_file_path=CSV_PATH_DATASET, model_name=models[0], results_path=results_path, knockout_heads=True, layer=layer, head=head)
